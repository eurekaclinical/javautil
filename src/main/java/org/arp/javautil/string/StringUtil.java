...
public class StringUtil {
	 ...
	 * @param str
	 *            a <code>String</code>.
	 * @return <code>true</code> if the string is <code>null</code>, of length 0, or
	 *         contains just whitespace; <code>false</code> otherwise.
	 */
	...
	public static List<String> escapeDelimitedColumns(List<String> columnValues, Map<String, String> replace,
			char delimiter) {
	...
	public static void escapeDelimitedColumnsInPlace(List<String> columnValues, Map<String, String> replace,
			char delimiter) {
	...
	public static void escapeDelimitedColumnsInPlace(String[] columnValues, Map<String, String> replace,
			char delimiter) {
		for (int i = 0; i < columnValues.length; i++) {
			String columnValue = columnValues[i];
	...
	public static void escapeAndWriteDelimitedColumns(String[] columnValues, char delimiter, Writer writer)
			throws IOException {
		escapeAndWriteDelimitedColumns(columnValues, null, delimiter, writer);
	}

	public static void escapeAndWriteDelimitedColumns(String[] columnValues, Map<String, String> replace,
	...
	public static void escapeAndWriteDelimitedColumns(List<String> columnValues, char delimiter, Writer writer)
			throws IOException {
		escapeAndWriteDelimitedColumns(columnValues, null, delimiter, writer);
	}

	public static void escapeAndWriteDelimitedColumns(List<String> columnValues, Map<String, String> replace,
			char delimiter, Writer writer) throws IOException {
		for (int i = 0, n = columnValues.size(); i < n; i++) {
	 ...
	 * {@link #escapeDelimitedColumn(java.lang.String, char)} because it does less
	 * temporary object creation. The performance difference will become more
	 * apparent when writing large delimited files.
	 *
	 * @param str
	 *            a column {@link String}.
	 * @param delimiter
	 *            the file's delimiter character.
	 * @param replace
	 *            replaces any keys that are found with the corresponding values.
	 * @param writer
	 *            the {@link Writer} to which to write the escaped column.
	 * @throws IOException
	 *             if an error writing to <code>writer</code> occurs.
	 */
	public static void escapeAndWriteDelimitedColumn(String str, char delimiter, Map<String, String> replace,
	 ...
	 *
	 * @param str
	 *            a column {@link String}. If <code>null</code>, the string
	...
	/**
	 * Escapes a column in a delimited file and writes a quoted string directly to a
	 * {@link Writer}. This is somewhat more efficient than
	 * {@link #escapeDelimitedColumn(java.lang.String, char)} because it does less
	 * temporary object creation. The performance difference will become more
	 * apparent when writing large delimited files.
	 *
	 * @param str
	 *            a column {@link String}. If <code>null</code>, an empty quoted string
	 *            ("") will be written out. If a quote is part of the string, it will be escaped 
	 *            by a second quote (my"Str will be written out as "my""Str"). If not, the
	 *            string will be surrounded with quotes and written out (myStr will be written
	 *            out as "myStr").
	 * @param delimiter
	 *            the file's delimiter character.
	 * @param writer
	 ...
	 *             if an error writing to <code>writer</code> occurs.
	 */
	public static void escapeAndWriteDelimitedColumn(String str, char delimiter, boolean addQuotes, Writer writer)
			throws IOException {
		if (str == null) {
			escapeAndWriteDelimitedColumn("", delimiter, true, writer);
		} else {
			if (addQuotes) {
				writer.write(QUOTE);
				writeStr(str, writer);			
